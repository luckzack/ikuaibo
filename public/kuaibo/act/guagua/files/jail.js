...
(function ( name, definition ){
	...
	*/
	function _loadImageIfVisible ( options, image, images ) {
		...
		// load the image if it is out the screen
		if(options.loadOutTheScreen){
			if( isVisible ){
				_loadImage( options, $img );	
			}
		}else{
	...
	*/
	...
	/* 
	* Main function 当加载完成后执行该方法 --> Load the images copying the "data-href" attribute into the "src" attribute
	*
	* @param options : configurations object
	* @param $img : image selected - jQuery obj
	*/
	function _loadImage ( options, $img ) {
		...
		//* restore image size
		if ( !!options.imgWidth ) {
			$img.attr("width",options.imgWidth);
		}
		//
